...
average_wine_rating = run_sql_file(
    "queries/average_wine_rating_per_country.sql", cursor
)
average_vintage_rating = run_sql_file(
    "queries/average_vintage_rating_per_country.sql", cursor
)
...
with st.sidebar:
    ...
    if st.button("Top ten wines"):
        table_container.subheader(
            "Top ten wines based on average rating of at least 4.6 and number of ratings."
        )
        table_container.dataframe(
            top_ten_wines,
            width=1000,
            column_config={1: "Wine", 2: "Number of ratings", 3: "Ratings average"},
        )
    if st.button("Prefered country for marketing"):
        table_container.subheader(
            "The choice of the prefered country for our marketing campaign based on the number of users"
        )
        table_container.dataframe(
            marketing_country,
            width=400,
            column_config={1: "Country", 2: "Number of users"},
        )
    if st.button("Best winery"):
        ...
        table_container.dataframe(
            best_winery,
            width=600,
            column_config={1: "Winery", 2: "Total number of ratings"},
        )
    if st.button("Wine flavors"):
        table_container.subheader(
            "Wines filtered based on these specific flavors: Coffee, Toast, Green apple, Cream and Citrus (975 results)."
        )
        table_container.dataframe(
            flavor_group,
            width=1000,
            column_config={1: "Wine", 2: "Keywords", 3: "Flavor group"},
        )
    if st.button("Top three grapes"):
        table_container.subheader(
            "Top three grapes based on the number of wines per grape"
        )
        table_container.dataframe(
            top_three_grapes,
            width=1000,
            column_config={1: "Grape", 2: "Number of wines"},
        )
    if st.button("Average rating for wines"):
        ...
        table_container.dataframe(
            ...
            width=1000,
            height=250,
            column_config={1: "Country", 2: "Average wine rating"},
        )
        wine_country, wine_rating = zip(
            *average_wine_rating
        )  # set data to plot bar chart
        fig, ax = plt.subplots()
    ...
    if st.button("Average rating for vintages"):
        ...
        table_container.dataframe(
            average_vintage_rating,
            width=1000,
            height=250,
            column_config={1: "Country", 2: "Average vintage rating"},
        )
        vintage_country, vintage_rating = zip(
            *average_vintage_rating
        )  # set data to plot bar chart
        fig, ax = plt.subplots()
